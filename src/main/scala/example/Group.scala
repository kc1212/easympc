...
import org.bouncycastle.math.ec.custom.djb.{Curve25519Point, Curve25519FieldElement, Curve25519}
...
object Group {
  ...
  implicit val curve25519: Group[TPt, TFe] = {
    new Group[TPt, TFe] {
      ...
      val b = curve.createPoint(new BigInteger("9"), new BigInteger("14781619447589544791020593568409986887264606134616475288964881837755586237401")).asInstanceOf[TPt]
      val multiplier = curve.getMultiplier
      override def add(a: TPt, b: TPt): TPt = a.add(b).asInstanceOf[TPt]
      override def mul(p: TPt, s: TFe): TPt = multiplier.multiply(p, s.toBigInteger()).asInstanceOf[TPt]
      override def base(): TPt = b
      override def rand(r: Rand): TFe = ???
